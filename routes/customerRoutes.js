...
router.get("/dashboard", async (req, res) => {
  try {
    console.log(
      ...
      req.user ? req.user.fullName : "Not logged in"
    );
    console.log(
      "Customer dashboard - Role:",
...
router.get("/services/:id", async (req, res) => {
  try {
    ...
    // Lấy đánh giá
    const reviews =
      (await Review.find({ service: service._id, isVisible: true })
        .populate("customer", "fullName avatar")
        .sort({ createdAt: -1 })) || [];
...
router.get("/cart", requireAuth, async (req, res) => {
  try {
    ...
    const services = await Service.find({ _id: { $in: serviceIds } }).populate(
      "shop",
      "shopName rating totalReviews"
    );

    const cartWithDetails = cart.map((cartItem) => {
      const service = services.find(
        (s) => s._id.toString() === cartItem.serviceId
      );
      return {
        ...cartItem,
        service,
...
router.post("/cart/add", requireAuth, async (req, res) => {
  try {
    ...
    const existingItem = req.session.cart.find(
      (item) => item.serviceId === serviceId && item.eventDate === eventDate
    );
    if (existingItem) {
...
router.post("/cart/update", requireAuth, (req, res) => {
  try {
    ...
    const item = req.session.cart.find(
      (i) => i.serviceId === serviceId && i.eventDate === eventDate
    );
    if (!item) {
      return res
        .status(404)
        .json({ success: false, message: "Không tìm thấy mục trong giỏ hàng" });
    }
...
router.post("/payment", requireAuth, async (req, res) => {
  try {
    ...
    if (eventDates.length > 1) {
      console.log(
        `Multiple dates found (${eventDates.length}), using primary date: ${primaryEventDate}`
      );
    }
    ...
    const order = new Order({
      ...
      notes:
        eventDates.length > 1
          ? `Đơn hàng gộp cho các ngày: ${eventDates.join(", ")}`
          : "",
      paymentMethod: "vnpay",
    ...
    try {
      ...
      return res.status(200).json({
        ...
        message:
          eventDates.length > 1
            ? `Đã gộp ${eventDates.length} ngày khác nhau thành 1 đơn hàng`
            : undefined,
      });
    } catch (error) {
...
router.post("/order", requireAuth, async (req, res) => {
  try {
    ...
    const formData = {
      ...
      contactPhone:
        cleanInput(req.body.contactPhone) || cleanInput(req.user.phone),
    ...
    if (isValidSignature) {
      ...
      if (vnpResponseCode === "00") {
        const updatedOrder = await Order.findByIdAndUpdate(
          orderId,
          {
            status: "pending", // Keep as pending until shop confirms
        ...
        // Update service sold counts after successful payment
        await incrementSoldCountForOrder(orderId);

        console.log("Order updated successfully:", updatedOrder);
        req.flash("success", "Thanh toán thành công");
      } else {
        await Order.findByIdAndUpdate(orderId, {
          status: "cancelled", // Changed from 'pending' to 'cancelled'
...
router.post(
  "/shop/orders/:id/request-confirm",
  requireShop,
  async (req, res) => {
    try {
      const order = await Order.findOne({
      ...
      if (!order) {
        req.flash(
          "error",
          "Đơn hàng không tồn tại hoặc bạn không có quyền thực hiện"
        );
        return res.redirect("/customer/shop/orders");
      }
...
router.post("/orders/:id/confirm", requireAuth, async (req, res) => {
  try {
    ...
    if (order.status !== "confirmed") {
      req.flash(
        "warning",
        "Đơn hàng chưa được shop xác nhận để yêu cầu hoàn tất"
      );
...
router.post("/profile/update", requireAuth, async (req, res) => {
  try {
    ...
    console.log("Final update data:", {
      ...updateData,
      password: updateData.password ? "***" : "not changed",
    });
    ...
    req.flash(
      "error",
      "Có lỗi xảy ra khi cập nhật thông tin: " + error.message
    );
    res.redirect("/customer/profile");
  }
});
...
router.post("/request-shop", requireAuth, async (req, res) => {
  try {
    const { shopName, description, address, phone, email, businessLicense } =
      req.body;
    ...
    req.flash(
      "success",
      "Yêu cầu trở thành Chủ shop đã được gửi. Admin sẽ xem xét và duyệt trong thời gian sớm nhất."
    );
    res.redirect("/customer/profile");
  } catch (error) {
